...
const VerificationCodeUser = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const res = await axios.post("https://cafesphere-2-backend.onrender.com/user/login/reset-password", { otp: verificationCode });

            if (res.data.success) {
                setMessage("Verification successful!");
                setTimeout(() => navigate("/login/forgot-password/verification-code/new-password"), 500);
                localStorage.setItem("resetToken", res.data.resetToken);
                const token = localStorage.getItem("resetToken");
                console.log("token in otp is -> ", token);
            } else {
